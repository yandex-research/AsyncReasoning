"""
Extended AsyncReasoningSolver for jailbreak evaluation with safety prompts and writer blocking.
"""
...
class JailbreakAsyncReasoningSolver:
    """
    Extended solver for jailbreak evaluation.

    Uses safety_prompting.py prompts and supports writer blocking
    for a specified number of tokens at the start of generation.
    """
    ...
    def __init__(
        ...
        **kwargs
        ...
        """
        Initialize the jailbreak solver.

        Args:
            model: The pretrained model.
            tokenizer: The tokenizer.
            forbidden_token_ix: Deprecated, use separate thinker/writer forbidden tokens.
            thinker_forbidden_token_ix: Token IDs forbidden for thinker output.
            writer_forbidden_token_ix: Token IDs forbidden for writer output.
            end_of_think_token_ix: Token IDs that signal end of thinking.
            use_fast_kernel: Whether to use fast kernels.
            writer_block_tokens: Number of tokens to block writer at start (force thinker-only).
            **kwargs: Additional arguments passed to model preparation.
        """
        if use_fast_kernel:
    ...
    def check_if_should_continue_writing(
        ...
        """Check if the thinker has enough information to continue writing."""
        ...
        yes_id = self.tokenizer(
            prompting.yes_token,
            **self.tokenizer_kwargs
        )["input_ids"].item()
        no_id = self.tokenizer(
            prompting.no_token,
            **self.tokenizer_kwargs
        )["input_ids"].item()
    ...
    def solve(
        ...
        For the first `writer_block_tokens` steps, forces State.thinker_only
        to allow the thinker to complete safety checks before writing begins.

        Args:
            problem: The input problem/prompt.
            display_generation_in_real_time: Whether to display generation progress.
            budget: Maximum number of generation steps.
            on_new_tokens_generated: Optional callback for new tokens.

        Returns:
            Tuple of (writer_output_str, thinker_output_str, token_times, eos_generated)
        """
            ...
            for step in range(budget):
                ...
                if cache.state != State.writer_only and \
                   (self.writer_block_tokens == 0 or step >= self.writer_block_tokens):
                    if ((step + 1) % 20 == 0 or self.is_end_of_step(thinker_output_tokens)):
                        should_write = self.check_if_should_continue_writing(
                            cache, prompting
                        )
                        cache.state = (
                            State.thinker_and_writer if should_write
                            else State.thinker_only
                        )
